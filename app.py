...
def page_forecast():
    ...
    forecast_days = st.slider(
        ...
        7, 180, int(default_forecast_days),
        key='forecast_days'
    )

    # Historical values
    last_known = series[['ds', 'y']].set_index('ds').y.sort_index()
    last_vals = last_known.values.tolist()

    # Feature builder
    def build_features_for_date(target_date, base_min_date, history):
        day_num = (target_date - base_min_date).days
        weekday = target_date.weekday()

        def get_lag(l):
            return history[-l] if len(history) >= l else np.mean(history) if history else 0
        ...
        return {
            ...
            'lag_7': get_lag(7),
            'lag_30': get_lag(30),
            'roll_7': np.mean(history[-7:]) if history else 0
        }
    ...
    feature_cols = [
        ...
        'weekday', 'is_weekend',
        'lag_1', 'lag_7', 'lag_30', 'roll_7'
    ]

    # 🔴 MAIN FIX: clamp negative predictions
    for fd in future_dates:
        feats = build_features_for_date(fd, series['ds'].min(), iter_history)
        Xf = pd.DataFrame([feats])[feature_cols]

        raw_pred = model.predict(Xf)[0]
        pred = max(0, raw_pred)   # ✅ NO NEGATIVE SALES
    ...
    fig_fore.update_layout(
        title=f"Forecast — {sel_prod} (next {forecast_days} days)",
    ...
    # Total forecast (always positive)
